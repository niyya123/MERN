...
function AdminUser() {
    ...
    const handleDelete = (id, user) => {
        ...
        handleDialog(
            `Bạn có chắc chắn muốn xóa ${user.isAdmin ? 'quản trị viên' : 'người dùng'} ${user.name} này không?`,
            true,
            `Xóa: ${user.isAdmin ? 'Quản trị viên ' : 'Người dùng '}` + userList[index].name,
        );
            ...
            <div className={cx('p-3')}>
                <div className={cx('d-flex align-items-center mb-4 qlsp')}>
                    <h1 className={cx('mr-3 fw-bold UserTitle')}>
                        <FontAwesomeIcon icon={faUserAlt}></FontAwesomeIcon> Quản lý người dùng
                    </h1>
                    ...
                    <h2 className={cx('fw-bold totalCountPur')}>
                        (<FontAwesomeIcon icon={faUserAlt}></FontAwesomeIcon> Tất cả: {userList.length} người dùng)
                    </h2>
                </div>
                <div>
                    <Link className={cx('btnDeleteAllUsers btn btn-info')} style={{"marginBottom":"10px"}} to={`/admin/users/createuser`}>
                        <FontAwesomeIcon icon={faAdd}></FontAwesomeIcon> Thêm người dùng
                    </Link>
                </div>
                    ...
                    <tbody>
                        {dataEachPage.map((user) => (
                            <tr key={user._id}>
                                ...
                                <td style={{ width: '8%' }}>
                                    {user.isAdmin === true ? 'Quản trị viên' : 'Người dùng'}
                                </td>
                                <td style={{ width: '8%' }}>
